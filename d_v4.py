...
# Define a function to draw center points on the bird's-eye view image
def draw_birdseye_view(center_points):

    birdseye_view = np.zeros((720, 1280, 3), dtype=np.uint8)
    for center in center_points:
        # Draw the center point on the bird's-eye view image
        cv2.circle(birdseye_view, center, 5, (0, 0, 255), -1)
...
while video_cap.isOpened():
    ...
    bird_eye_view2  = np.zeros((1440, 2560, 3), dtype=np.uint8)
    ...
    if det is not None and len(det):
        ...
        for id1, id2 in itertools.combinations(unique_ids, 2):
            ...
            # bird_eye_view = cv2.circle(bird_eye_view, (int(center2[0]),int(center2[1])), 5, (0, 0, 255), -1)
            # #cv2.line(bird_eye_view, (int(center1[0]), int(center1[1])), (int(center2[0]), int(center2[1])), (0,255,20), 6)
